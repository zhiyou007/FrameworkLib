...
/**
 * Created by zhiyou007 on 2015/10/8.
 */
...
public class OkHttpClientManager
{
    ...
    public static void postJsonAsyn(String url, String bodyStr, final ResultCallback callback, Object tag)
    {
        getInstance().getPostDelegate().postJsonAsyn(url, bodyStr, callback, tag);
    }

    public static void postJsonAsyn(String url, String bodyStr, final ResultCallback callback, Object tag, long timeout)
    {
        getInstance().getPostDelegate().postJsonAsyn(url, bodyStr, callback, tag, timeout);
    }

    //同步的post
    public static String postJson(String url, String bodyStr) throws IOException
    {
        return getInstance().getPostDelegate().postJson(url, bodyStr).body().string();
    }
    ...
    private void deliveryResult(ResultCallback callback, Request request, long timeout)
    {
        if (callback == null) callback = DEFAULT_RESULT_CALLBACK;
        final ResultCallback resCallBack = callback;
        //UI thread
        callback.onBefore(request);
        OkHttpClient client = mOkHttpClient.clone(); // Clone to make a customized OkHttp for this request.
...
//        client.setReadTimeout(TIMEOUT, TimeUnit.MILLISECONDS);
    ...
    public class PostDelegate
    {
        ...
        public Response postJson(String url, String bodyStr) throws IOException
        {
            RequestBody body = RequestBody.create(MediaType.parse("application/json;charset=utf-8"), bodyStr);
            Request request = buildPostRequest(url, body, null);
            Response response = mOkHttpClient.newCall(request).execute();
            return response;
        }
        ...
        public void postJsonAsyn(String url, String bodyStr, final ResultCallback callback, Object tag)
        {
            postAsynWithMediaType(url, bodyStr, MediaType.parse("application/json;charset=utf-8"), callback, tag);
        }

        public void postJsonAsyn(String url, String bodyStr, final ResultCallback callback, Object tag, long timeout)
        {
            postAsynWithMediaType(url, bodyStr, MediaType.parse("application/json;charset=utf-8"), callback, tag, timeout);
        }
        ...
        public void postAsynWithMediaType(String url, String bodyStr, MediaType type, final ResultCallback callback, Object tag, long timeout)
        {
            RequestBody body = RequestBody.create(type, bodyStr);
            Request request = buildPostRequest(url, body, tag);
    ...
    public class DownloadDelegate
    {
        ...
        public void downloadAsyn(final String url, final String destFileDir, final String filename, final ResultCallback callback, Object tag)
        {
            final Request request = new Request.Builder()
                    .url(url)
                    .tag(tag)
        ...
        public void downloadAsyn(final String url, final String destFileDir, final String filename, final ResultCallback callback)
        {
            downloadAsyn(url, destFileDir,filename, callback, null);
        }
